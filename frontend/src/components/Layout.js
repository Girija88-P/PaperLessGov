...
import { Home, FileText, Users, Building2, History, Settings, LogOut, BarChart3, PlusCircle, ChevronDown, ChevronRight, GitBranch, CheckSquare, Send, Globe } from 'lucide-react';
import { useGovLang } from '../govI18n';
...
const Layout = ({ setAuth }) => {
  ...
  const { lang, t, changeLang, govTranslations } = useGovLang();
      ...
      <aside className="sidebar" style={{ width: '220px', display: 'flex', flexDirection: 'column', gap: 0, padding: '20px 12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '28px', paddingLeft: '4px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>{t.digitalWorkflow}</div>
          </div>
        </div>

        {/* Nav */}
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '2px', flex: 1 }}>
          <NavLink to="/dashboard" end style={nav}><Home size={15} /> {t.dashboard}</NavLink>
          <NavLink to="/users" style={nav}><Users size={15} /> {t.usersRoles}</NavLink>

          {localStorage.getItem('user_dept') === 'Common' && (
            <NavLink to="/departments" style={nav}><Building2 size={15} /> {t.departments}</NavLink>
          )}
          <NavLink to="/audit-logs" style={nav}><History size={15} /> {t.auditLogs}</NavLink>
          <NavLink to="/requests" style={nav}><Send size={15} /> {t.requests}</NavLink>
          <NavLink to="/settings" style={nav}><Settings size={15} /> {t.settings}</NavLink>
        </nav>
        ...
        <header style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', padding: '16px 32px', background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid var(--border)', backdropFilter: 'blur(10px)', sticky: 'top', zIndex: 100 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <select value={lang} onChange={e => changeLang(e.target.value)} style={{ padding: '5px 8px', borderRadius: '6px', border: '1px solid var(--border)', fontSize: '0.8rem', cursor: 'pointer', background: 'var(--card-bg)', color: 'var(--text-primary)' }}>
                {Object.keys(govTranslations).map(code => (
                  <option key={code} value={code}>{govTranslations[code].name}</option>
                ))}
              </select>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(59,130,246,0.1)', color: 'var(--primary)', padding: '6px 16px', borderRadius: '20px', fontSize: '0.85rem', fontWeight: '700', border: '1px solid rgba(59,130,246,0.2)' }}>
              <Building2 size={14} /> {localStorage.getItem('user_dept') === 'Common' ? t.mainGov : (localStorage.getItem('user_dept') || t.stateGov)}
            </div>
          </div>
        </header>
