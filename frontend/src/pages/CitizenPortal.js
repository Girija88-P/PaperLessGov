...
import api from '../api';
import { Search, FileText, CheckCircle, Clock, User, LogOut, ChevronRight, MapPin, Globe } from 'lucide-react';
      ...
      <main style={{ padding: '40px 5%' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          {activeTab === 'tracking' ? (
            ...
            <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', overflow: 'hidden' }}>
              <div style={{ padding: '24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h2 style={{ fontSize: '1.4rem', margin: 0 }}>{t.officialNotif}</h2>
                <span style={{ fontSize: '0.95rem', color: '#64748b' }}>{notifications.filter(n => !n.is_read).length} {t.unread}</span>
              </div>
              <div style={{ padding: '24px' }}>
                {notifications.length === 0 ? (
                  <p style={{ color: '#64748b', textAlign: 'center', padding: '40px', fontSize: '1.05rem' }}>{t.noNotif}</p>
                ) : (
                  notifications.map((n) => {
